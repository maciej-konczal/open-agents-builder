...
export async function PUT(request: NextRequest, response: NextResponse) {
    ...
    const apiResult = await genericPUT<KeyDTO>(await request.json(), keyDTOSchema, new ServerKeyRepository(requestContext.emailHash), 'keyLocatorHash');
    return Response.json(apiResult, { status: apiResult.status });
}

export async function GET(request: NextRequest, response: NextResponse) {
    const requestContext = await authorizeRequestContext(request, response);
    return Response.json(await genericGET<KeyDTO>(request, new ServerKeyRepository(requestContext.emailHash)));
}
