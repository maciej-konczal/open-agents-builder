...
export async function PUT(request: NextRequest, response: NextResponse) {
    ...
    const apiResult = await genericPUT<RecordDTO>(inputObj, recordDTOSchema, new ServerRecordRepository(requestContext.emailHash), 'id');
...
export async function GET(request: NextRequest, response: NextResponse) {
    const requestContext = await authorizeRequestContext(request, response);
    return Response.json(await genericGET<RecordDTO>(request, new ServerRecordRepository(requestContext.emailHash)));
}
