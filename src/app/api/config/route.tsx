...
export async function PUT(request: NextRequest, response: NextResponse) {
    ...
    const apiResult = await genericPUT<ConfigDTO>(await request.json(), configDTOSchema, new ServerConfigRepository(requestContext.emailHash), 'key');
    return Response.json(apiResult, { status: apiResult.status });
}

export async function GET(request: NextRequest, response: NextResponse) {
    const requestContext = await authorizeRequestContext(request, response);
    return Response.json(await genericGET<ConfigDTO>(request, new ServerConfigRepository(requestContext.emailHash)));
}

// clear all configuration - it's not used anywhere in the app so commented out for now
// export async function DELETE(request: NextRequest, response: NextResponse) {
//     const requestContext = await authorizeRequestContext(request, response);
//     const allConfigs = await genericGET<ConfigDTO>(request, new ServerConfigRepository(requestContext.emailHash));
//     if(allConfigs.length <= 1){
//         return Response.json({ message: "Cannot delete the last configuration", status: 400 }, {status: 400});
//     } else {
//         const deleteResults = [];
//         for(const config of allConfigs){
//             deleteResults.push(await genericDELETE(request, new ServerConfigRepository(requestContext.emailHash), { key: config.key}));
//         }
