...
export function OrderDeleteDialog({ order }: { order: Order}) {
      ...
      <AlertDialog>
        <AlertDialogTrigger>
          <Button className="ml-auto right-20 mr-2" size={"sm"} variant="secondary" onClick={() => {
            ...
            <Trash2Icon className="w-4 h-4 mr-2" />{t('Delete order')}
          </Button>
        ...
        <AlertDialogContent className="w-[400px] bg-background text-sm p-4 rounded-lg shadow-lg border border-gray-200">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {t('This action cannot be undone. This will permanently delete your order data') }    
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            <AlertDialogAction className='' onClick={async (e) => 
              {
                try {
                    const resp = await orderContext.deleteOrder(order)
                    if (resp.status === 200 ) {
